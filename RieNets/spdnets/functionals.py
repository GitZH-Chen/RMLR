...
class sym_Glogm(Function):
    ...
    def value(s : Tensor, param:Tensor = None) -> Tensor:
        # ensure that the eigenvalues are positive
        return s.clamp(min=EPS[s.dtype]).log().mul(param)

    @staticmethod
    def derivative(s : Tensor, param:Tensor = None) -> Tensor:
        # compute derivative
        sder = s.reciprocal().mul(param)
        # pick subgradient 0 for clamped eigenvalues
        sder[s<=EPS[s.dtype]] = 0
        return sder

    @staticmethod
    def forward(ctx: Any, M: Tensor, weight: Tensor, ensure_symmetric : bool = False) -> Tensor:
        X, s, smod, U = sym_modeig.forward(M, sym_Glogm.value, ensure_symmetric=ensure_symmetric,fun_param=weight)
        ctx.save_for_backward(s, smod, U,weight)
        return X

    @staticmethod
    def backward(ctx: Any, dX: Tensor):
        s, smod, U, weight = ctx.saved_tensors
        dz = sym_modeig.backward(dX, s, smod, U, sym_Glogm.derivative, fun_der_param=weight)
        # calculating dw
        sigma_log = sym_Glogm.value(s,1)
        tmp = U.transpose(-1, -2).matmul(dX).matmul(U)
        if s.dim()==1:
            dw = tmp.diagonal(dim1=-2, dim2=-1).mul(sigma_log)
        else:
...
class sym_Gexpm(Function):
    """
    Computes the matrix exponential for a batch of symmetric matrices.
    weight: \frac{1}{\ln(\alpha)}
    """
    @staticmethod
    def value(s : Tensor, param:Tensor = None) -> Tensor:
        return torch.exp(s.div(param))  # e^ (x * \ln(\alpha))

    @staticmethod
    def derivative(s : Tensor, param:Tensor = None) -> Tensor:
        return torch.exp(s.div(param)).div(param)  # a^x \ln(\alpha)

    @staticmethod
    def forward(ctx: Any, M: Tensor, weight: Tensor, ensure_symmetric : bool = False) -> Tensor:
        X, s, smod, U = sym_modeig.forward(M, sym_Gexpm.value, ensure_symmetric=ensure_symmetric,decom_mode='eigh',fun_param=weight)
        ctx.save_for_backward(s, smod, U, weight)
        return X
    ...
    def backward(ctx: Any, dX: Tensor):
        s, smod, U, weight = ctx.saved_tensors
        dz = sym_modeig.backward(dX, s, smod, U, sym_Gexpm.derivative,fun_der_param=weight)
        # calculating dw
        internal = smod.div(torch.pow(weight, 2)).mul(-s)
        tmp = U.transpose(-1, -2).matmul(dX).matmul(U)
        dw = tmp.diagonal(dim1=-2, dim2=-1).mul(internal).squeeze().mean(dim=0)
        return dz,dw
